...
import { searchUsersService } from "../services/communityService";
import {
...
export default function Messages() {
    ...
    const loadConversations = async (silent = false) => {
        ...
        try {
            ...
            if (res.success) {
                ...
                if (targetUserId && !activeConversation && !silent) {
    ...
    return (
        <div className="min-h-screen bg-[#FAF9F5] text-[#1E3A23] font-sans flex flex-col">
            <Navbar />

            {/* Main Messenger Dashboard Area */}
            <div className="flex-1 max-w-6xl w-full mx-auto p-4 md:py-6 flex flex-col md:flex-row gap-4 h-[calc(100vh-80px)] overflow-hidden">
                ...
                <div className={`flex-1 rounded-2xl border border-[#EBE8DB] bg-white shadow-md flex flex-col overflow-hidden ${
                    ...
                    {activeConversation ? (
                        ...
                        <div className="text-center py-20 px-4 text-[#6D7A6F] space-y-3">
                            <div className="h-16 w-16 bg-[#EBF5EE] rounded-full flex items-center justify-center mx-auto text-[#1E3A23] border border-[#D1E2D3]">
                                <MessageSquare size={32} />
                            </div>
                            <h3 className="font-serif text-lg font-bold text-[#1E3A23]">Your Conversations</h3>
                            <p className="text-xs max-w-xs mx-auto leading-relaxed">
                                Choose an explorer from the sidebar list or search to start a direct private discussion.
                            </p>
                            <button
                                onClick={() => setShowSearchModal(true)}
